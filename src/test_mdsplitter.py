...
class TestMDSplitter(unittest.TestCase):
    ...
    def test_include_delim_char(self):
        node = TextNode("This should have **\*asterix and bold**", TextType.NORMAL)
        new_nodes = split_nodes_delimitter([node], "**", TextType.BOLD)
        print(new_nodes)
        self.assertEqual(new_nodes, [TextNode("This should have ", TextType.NORMAL), TextNode("\*asterix and bold", TextType.BOLD)])
